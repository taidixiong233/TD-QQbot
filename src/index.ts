		...
		for (let p of argv) {
			switch (p) {
				...
				case '-repairpluginjson':
					...
					break
				default:
					let version = JSON.parse(fs.readFileSync(path.join(__dirname, '../package.json')).toString()).version
...
`)
	...
	async function _login() {
		for (let i in config.qqconfig) {
			f.putlog(`正在登录第${Number(i) + 1}个账号,共${config.qqconfig.length}个`)
			await login(config.qqconfig[i].uin, config.qqconfig[i].pwd, config.qqconfig[i].platform)
		}
	...
	if (settings.print_pluginlist) {
		...
		let success: { 插件名称: string, 作者名称: string, 版本: string, 入口文件: string, 站点: string }[] = []
		let error: { 插件名称: string, 作者名称: string, 版本: string, 入口文件: string, 站点: string, 错误原因: string }[] = []
