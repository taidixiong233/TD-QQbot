...
export function ts2time(timestamp: number): string {
  const add0 = (m: number): string => (m < 10 ? "0" + String(m) : String(m));
...
export const getTime_format = (
  ...
  if (time.getMilliseconds() < 10)
    milliseconds = "00" + String(time.getMilliseconds());
  else if (time.getMilliseconds() < 100)
    milliseconds = "0" + String(time.getMilliseconds());
