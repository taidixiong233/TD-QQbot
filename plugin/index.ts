...
export async function Plugin_start(
  ...
  log(
    `共${plugin.error.length + plugin.success.length}个插件,成功识别了${
      plugin.success.length
    }个,失败${plugin.error.length}个`
  );
  ...
  for (let p of plugin.success) {
    try {
      await import(path.join(__dirname, p.start_filename))
        .then((plugin) => {
  ...
  setTimeout(
    () =>
      log(
        `尝试加载了${successarr.length + errorarr.length}个插件, 成功${
          successarr.length
        }个, 失败${errorarr.length}个`
      ),
    200
  );
}
